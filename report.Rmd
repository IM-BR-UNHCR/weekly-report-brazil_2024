...
subtitle: WEEK `r if( lubridate::isoweek(lubridate::today()) == 1) {53} else { lubridate::isoweek(lubridate::today())}  `/`r lubridate::year(lubridate::today())-1`
...
data <- 
  ...
  select(id = `_id`,
         ...
         new_trends.narrative = `group_gt36y03/NON_CONFIDENTIAL_PR_FOCUS_ON_NEW_TRENDS`,
          new_trends.confidential = `group_gt36y03/CONFIDENTIAL_PROVID_FOCUS_ON_NEW_TRENDS`,
...
data <- 
  ...
  mutate(office = factor(office),
         section = factor(section, levels = c("Context + Situation", "Population Trends", "Protection", "Indigenous Peoples", "Resettlement and Complementary Pathways", "Shelter", "Local_Integration", "CBI")))
...
# isoweek(ymd("2012-03-26"))
  ...
  datanow <- data %>% filter(year == (isoyear(today())-1), week == 53)
