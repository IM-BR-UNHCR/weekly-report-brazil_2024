...
author:
  - name: Chiara Orsini and Vivianne Barbosa Soares
    ...
    email: orsini@unhcr.org & barbosas@unhcr.org
output:
  pagedown::html_paged:
...
data <- 
  ...
  select(id = `_id`,
         ...
         office.unit = `group_dv7hn13/Office_Unit`,
...
data <- 
  ...
  pivot_longer(-c(id, year, week, office.unit), names_pattern = "(.+)\\.(.+)", names_to = c("section", "dimension")) %>% 
  pivot_wider(names_from = "dimension", values_from = "value")


data <- 
  data %>% 
  mutate(office.unit = case_when(office.unit == "so_boa_vista" ~ "SO Boa Vista",
                             office.unit == "livelihoods___bras_lia" ~ "Livelihoods - Brasília",
                             office.unit == "protection___bras_lia" ~ "Protection - Brasília",
                             office.unit == "resettlement_and_complementary_paths___b" ~ "Resettlement and Complementary Pathways",
                             office.unit == "indigenous___bras_lia" ~ "Indigenous – Brasília",
                             office.unit == "cbi___bras_lia" ~ "CBI – Brasília ",
                             office.unit == "psp___s_o_paulo" ~ "PSP – São Paulo",
                             office.unit == "fo_s_o_paulo" ~ "FO São Paulo",
                             office.unit == "fp_bel_m" ~ "FU Belém",
                             office.unit == "fo_manaus" ~ "FO Manaus",
                             office.unit == "fu_pacaraima" ~ "FU Pacaraima"),
...
data <- 
  data %>% 
  mutate(office.unit = factor(office.unit),
         section = factor(section, levels = c("Context + Situation", "Population Trends", "Protection",
                                              "Indigenous Peoples", "Resettlement and Complementary Pathways", "Shelter", "Internal Relocation", "Livelihoods", "CBI")))


data <- 
  data %>% 
  select(id, year, week, office.unit, section, narrative, confidential) %>%
...
render_office.unit <- function(data, .office.unit) {
  cat(glue::glue("## {.office.unit}"), sep = "\n")
  data <- data %>% filter(office.unit == .office.unit)
