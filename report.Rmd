...
subtitle: WEEK `r if( lubridate::isoweek(lubridate::one_week_ago_datey()) == 1) {52} else { lubridate::isoweek(lubridate::one_week_ago_date())}  `/`r lubridate::year(lubridate::one_week_ago_date())`
...
if(isoweek(today()) ==1) {
  ...
  datanow <- data %>% filter(year == (isoyear(one_week_ago_date())), week == 52)
} else {
  datanow <- data %>% filter(year == isoyear(one_week_ago_date()), week == isoweek(one_week_ago_date()))
}
