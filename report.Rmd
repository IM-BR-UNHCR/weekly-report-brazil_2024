...
subtitle: WEEK `r if( lubridate::isoweek(lubridate::today()) == 1) {54} else { lubridate::isoweek(lubridate::today())}  `/`r lubridate::year(lubridate::today())-1`
...
if(isoweek(today()) == 1) {
  ...
  datanow <- data %>% filter(year == (isoyear(today())-1), week == 54)
} else {
  datanow <- data %>% filter(year == isoyear(today()-1), week == isoweek(today()))
}
