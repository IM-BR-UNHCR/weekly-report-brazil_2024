...
subtitle: WEEK `r if( lubridate::isoweek(lubridate::today()) == 1) {52} else { lubridate::isoweek(lubridate::today())-1}  `/`r lubridate::year(lubridate::today()-1)`
date: Brasilia Office - Brazil
author:
  - name: Vivianne Barbosa Soares
    affiliation: Senior Reporting Assistant
...
data <- 
  ...
  mutate(office = case_when(office == "so_boa_vista" ~ "SO Boa Vista",
         ...
         section = c(work_operation = "Context + Situation", 
                      ...
                      cbi = "CBI")[section],
             narrative = replace_na(narrative, "Nothing non-confidential to report."),
...
```{r, results='asis'}
purrr::walk(levels(fct_drop(datanow2$office)), ~render_office(datanow2, .))
```
